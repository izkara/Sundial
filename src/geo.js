...
static void path_layer_update_callback(Layer *me, GContext *ctx) {
	...
	int shadowX = x + (int)(50.0 * _cos(DR(thla[currentHour - 5])));
	int shadowY = y + (int)(50.0 * _sin(DR(thla[currentHour - 5])));

	APP_LOG(APP_LOG_LEVEL_ERROR, "HOUR: %d", currentHour);
	APP_LOG(APP_LOG_LEVEL_ERROR, "HOUR: %f", thla[currentHour - 5]);


	graphics_draw_line(ctx, GPoint(shadowX, shadowY), GPoint(x, y));
	graphics_draw_line(ctx, GPoint(shadowX, shadowY), GPoint(x, y - 30));
